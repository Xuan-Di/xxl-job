...
//    定义日志 镀锡
     ...
     * @param executorShardingParam  分片广播参数
     * @param executorParam  任务参数   执行器，
     *          null: use job param
...
//            执行   调度方法
     ...
     *
     * 正式的调度
     * @param group        项目，注册列表可能为空             job group, registry list may be empty
     * @param jobInfo   任务信息
...
//        如果地址不为空
        ...
        // 5、collection trigger info  ， 日志信息拼接
        ...
        // 6、save log trigger-info  ，存储任务执行日志信息
        jobLog.setExecutorAddress(address);  //  执行器地址，本次执行的地址
        jobLog.setExecutorHandler(jobInfo.getExecutorHandler());// JobHandler 就是方法名称
        jobLog.setExecutorParam(jobInfo.getExecutorParam());// 执行  参数
        jobLog.setExecutorShardingParam(shardingParam);// 分片
        jobLog.setExecutorFailRetryCount(finalFailRetryCount);//   执行失败 重试 次数
...
//        更新日志信息
    ...
    /**
     * run executor    远程 调用项目 ，进行执行对应的方法
     * @param triggerParam   触发的参数，也就是任务里的各种信息
     * @param address  调用的远程地址 调用我们的项目
     * @return
     */
